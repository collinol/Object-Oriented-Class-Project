...
class Client{
	...
	public void sellItem() throws FileNotFoundException{
        ...
        while(true)
        {
            System.out.print("Set start Price: ");
	...
	public returnBidPair bid() {
        ...
        while (true) {
            ...
            if(choice.charAt(0)=='q'){choice = choice.toString();}
            switch (choice) {
                case "browse":
                    server.listItems();
                    break;
                ...
                default:
                    ...
                    if (itemIdSelect <= server.items.size() && server.items.get(itemIdSelect - 1).getSellerId() == this.clientId){
                        ...
                        else if (bidPrice <= currentPrice && !itemSelected.getItemExpiration()) {
                            while (true){
                                ...
                                if (reBidPriceString.charAt(0) == 'q'){
                                ...
                                if (reBidPrice > currentPrice && itemSelected.getSellerId() != this.clientId && !itemSelected.getItemExpiration()) {
                                    ...
                                    System.out.println("Client " + clientId + " currently has the highest bid on item " + itemIdSelect);
                                    itemSelected.setWinningBidderId(clientId);
                                    itemSelected.setCurrentPrice(reBidPrice );
                                    rbp = new returnBidPair(true,itemSelected.getItemId());
                                    return rbp;
                                }
